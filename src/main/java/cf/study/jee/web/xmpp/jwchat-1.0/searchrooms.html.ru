...
<html>
  <head>
    <title>JWChat - Искать комнаты</title>
...
function doSub() {
	...
	if (conference_server.indexOf('@') != -1 || conference_server.indexOf('/') != -1) {
		alert("это непохоже на правильное имя хоста");
		return false;
	}
	...
	document.getElementById("search_result_header").innerHTML = "Результаты поиска для "+conference_server;
	document.getElementById("search_result_info").innerHTML = "Выполняется запрос к "+conference_server+". Пожалуйста подождите...";
...
function getDiscoItems(iq) {
	if (!iq || iq.getType() != 'result') {
		document.getElementById("search_result_info").innerHTML = "Произошла ошибка!";
		if (iq)
	...
	var header = new Array("Комната","Описание");
	var cell; 
...
function getVCard(iq) {
	...
	// we're done tell frame about it
		...
		document.getElementById("search_result_info").innerHTML = "Закончено.";
	}
}
...
onunload = cleanUp;
  ...
  <body style="margin:8px;">
		<div id="search_tab">
			<h2>Искать комнаты</h2>
			<form name="sub" onsubmit="return doSub();">
				<table width="100%">
						<tr>
							<td nowrap>
								<label for="conference_server">Искать на сервере:
							</td>
							<td width="100%">
		...
		<table id="search_result_tab" width="100%" height="100%">
				...
				<tr>
					<td align=right>
						<button id="addbookmark_button" onClick="return addBookmark();" disabled>Закладка</button>&nbsp;<button id="join_room_button" onClick="return joinRoom();">Присоединиться</button>&nbsp;<button onClick="return backtosearch();">Назад</button>
					</td>
				</tr>
		</table>
  </body>
