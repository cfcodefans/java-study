...
onlstatus["chat"] = "chat";
onlstatus["away"] = "afwezig";
onlstatus["xa"] = "langdurig afwezig";
onlstatus["dnd"] = "niet storen";
onlstatus["invisible"] = "onzichtbaar";
...
function openGroupchat(aJid,nick,pass) {
  ...
  try {
  if(!user) {
    user = roster.addUser(new RosterUser(aJid,'',["Chatruimtes"],aJid.substring(0,aJid.indexOf('@'))));
    user.type = 'groupchat';
    roster.print();
  }
...
function handlePresence(presence) {
  ...
  case 'unsubscribe':
    if (!isGateway(from))
      alert("Uw inschrijving op de aanwezigheid van "+presence.getFrom()+" is verwijderd."); /* [TODO] don't use alert here */
    break;
  case 'error':
    var user = roster.getUserByJID(from);
  ...
  if (user.roster && x) {
	 ...
	 */
	if (confirm("Een nieuwe chatruimte werd aangemaakt en moet nog geconfigureerd worden door u. Wilt u de configuratie nu aanpassen?\nOpmerking: Klik op 'Annuleren' om de standaardinstellingen te gebruiken!"))
	...
	oMsg.setBody(""+ouser.name+" heet nu "+htmlEnc(ouser.nick));
	user.chatmsgs = user.chatmsgs.concat(oMsg);
	if (user.chatW && !user.chatW.closed && user.chatW.popMsgs)
	  user.chatW.popMsgs();
	...
	var body;
	if (ouser.actor)
	  body = ""+ouser.name+" werd verbannen door "+ouser.actor;
	else
	  body = ""+ouser.name+" werd verbannen";
	if (ouser.reason)
	  body += ": " + ouser.reason;
	oMsg.setBody(body);
	user.chatmsgs = user.chatmsgs.concat(oMsg);
	...
	var body;
	if (ouser.actor)
	  body = ""+ouser.name+" werd gekickt door "+ouser.actor;
	else
	  body = ""+ouser.name+" werd gekickt";
	if (ouser.reason)
	  body += ": " + ouser.reason;
	oMsg.setBody(body);
	user.chatmsgs = user.chatmsgs.concat(oMsg);
    ...
    if (!user.roster.getUserByJID(presence.getFrom()) && !nickChanged) {
      ...
      var oMsg = new JSJaCMessage();
      oMsg.setFrom(user.jid);
      oMsg.setBody(""+ouser.name+" betrad de chatruimte");
      user.chatmsgs = user.chatmsgs.concat(oMsg);
      if (user.chatW && !user.chatW.closed && user.chatW.popMsgs)
	user.chatW.popMsgs();			
      ...
      var oMsg = new JSJaCMessage();
      oMsg.setFrom(user.jid);
      var body = ""+ouser.name+" verliet de chatruimte";
...
function handleConError(e) {
  ...
  case '401':
    alert("De autorisatie mislukte");
    if (!con.connected())
      window.close();
    break;
  case '409':
    alert("De registratie mislukte!\n\nProbeer opnieuw met een andere gebruikersnaam!");
    break;
  case '503':
    alert("Dienst niet beschikbaar");
    break;
  case '500':
    if (!con.connected() && !logoutCalled && onlstat != 'offline')
      if (confirm("Interne serverfout.\n\nVerbinding verbroken.\n\nWilt u opnieuw verbinden?"))
...
if (!pass) {
	alert("Het wachtwoord ontbreekt.\nBezig met afbreken...");
	window.close();
}
...
//-->
  ...
  <body>
    Uw webbrowser moet frames en javascript ondersteunen. Raadpleeg de <a href="help.html">handleiding</a> om te zien welke webbrowsers ondersteund worden.
  </body>
</html>
