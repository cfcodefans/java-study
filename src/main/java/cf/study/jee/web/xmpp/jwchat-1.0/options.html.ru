...
function service_selected(el) {
  ...
  pane.innerHTML = "<div class='transport_pane'>"+"Запрос к сервису...<br>Пожалуйста подождите!"+"</div>";
...
function getRegisterForm(iq) {
  if (!iq || iq.getType() != 'result' || iq.getQueryXMLNS() != 'jabber:iq:register') {
    pane.innerHTML = "<div class='transport_pane'>"+"Произошла ошибка..."+"</div>";		
    return;
  }
  ...
  if (iq.getNode().getElementsByTagName('x').length && iq.getNode().getElementsByTagName('x').item(0).getAttribute('xmlns') == 'jabber:x:data') {
    ...
    if (query.getElementsByTagName('registered').item(0))
      html += "<div class='pane_el'><b style='color:red;'>"+"Вы уже зарегистрированы на этом сервисе!"+"</b></div>";
  ...
  if (query.getElementsByTagName('registered').item(0)) {
    html += "<button onClick=\"unregisterGateway('"+iq.getFrom()+"');\">"+"Отключиться"+"</button>";
    html += "&nbsp;";
    html += "<button onClick=\"registerService('"+iq.getFrom()+"','"+pane.id+"');\">"+"Обновить"+"</button>";
  } else
    html += "<button onClick=\"registerService('"+iq.getFrom()+"','"+pane.id+"');\">"+"Зарегистрироваться"+"</button>";
  html += "</div>";
  pane.innerHTML = html;
...
function changePassword(form) {
  ...
  if (form.password_new.value != form.password_repeat.value)
    return show_error('password_tab', "Пароли не совпадают");
  ...
  result_handler:
    function() {
      me.show_error('password_tab', "Пароль успешно сменён");
    },
  error_handler:
    function() {
...
function registerService(sJid,paneid) {
  ...
  pane.innerHTML = "<strong>"+"Отправлен запрос к "+sJid+"..."+"</strong><br>";
}
...
function handleRegisterService(iq,sJid) {
  if (iq.getType() == 'error') {
    ...
    var html = "<strong style='color:red;'>"+"Ошибка";
	...
	case 'bad-request': html += "Неверный запрос";
	  break;
  ...
  pane.innerHTML += "<strong>"+"Регистрация прошла успешно."+"</strong><br/>";
	
  // send presence
  pane.innerHTML += "<strong>"+"Отправляется информация о присутствии..."+"</strong><br />";
  var aPresence = new JSJaCPresence();
  aPresence.setTo(sJid);
  aPresence.setShow(srcW.onlstat);
  aPresence.setStatus(srcW.onlmsg);
  srcW.con.send(aPresence);
  pane.innerHTML += "<strong>"+"Готово."+"</strong><br />";
}
...
function unregisterGateway(sJid) {
  ...
  pane.innerHTML = "<strong>"+"Отключаемся от сервиса "+sJid+" ..."+"</strong><br />";
  var iq = new JSJaCIQ();
  iq.setType('set');
  iq.setTo(sJid);
  var query = iq.setQuery('jabber:iq:register');
...
function handleUnregisterService(iq,sJid) {
  ...
  pane.innerHTML += "<strong>"+"Готово."+"</strong><br />";
  pane.innerHTML += "<strong>"+""+sJid+" удаляется из списка..."+"</strong><br />";
  ...
  if (confirm("Удалить все связанные контакты?")) {
    pane.innerHTML += "<strong>"+"Удаляются контакты..."+"</strong><br />";
    var iq = new JSJaCIQ();
    iq.setType('set');
    var query = iq.setQuery('jabber:iq:roster');
    for (var i=0; i<srcW.roster.users.length; i++) {
...
onunload = savePrefs;
  ...
  <body style="margin: 8px;">
	...
	<table border=0 cellpadding=0 cellspacing=0 height="100%" width="100%">
		<tr>
			<td>
			<div id="tablabels" class="tablabels"><span class="tablabelactive">Общие</span><span class='tablabel' id='pw_label'>Сменить пароль</span><span class="tablabel" id='gw_label'>Шлюзы</span><span class="tablabel" id='dir_label'>Список пользователей</span><span class="tablabel">Информация</span></div>
			</td></tr>
...
<script language="JavaScript">
			...
			</div><div class="tabinactive" id="transports_tab">Зарегистрироваться на <select id="transports_selector" onchange="service_selected(this);"><option value=""> - Выберите шлюз - </option></select><div id="transport_pane"></div></div><div class="tabinactive" id="directory_tab">Зарегистрироваться на  <select id="directory_selector" onchange="service_selected(this);"><option value=""> - Выберите директорию - </option></select><div id="directory_pane"></div></div><div class="tabinactive" id="about_tab" align="center">
								<p>
