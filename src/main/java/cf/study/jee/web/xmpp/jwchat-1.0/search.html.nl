...
function searchUser() {
  ...
  document.getElementById('results_header').innerHTML = "Bezig met opvragen...";
  // send query
  var searchstring = '';
...
function getQueryResult(iq) {
  if (!iq || iq.getType() != 'result') {
    document.getElementById('results_header').innerHTML = "Er deed zich een fout voor";
    return false;
  }
  ...
  if (iq.getNode().getElementsByTagName('x').length && iq.getNode().getElementsByTagName('x').item(0).getAttribute('xmlns') == 'jabber:x:data') {
    ...
    if (x.getElementsByTagName('title').item(0))
      ...
      document.getElementById('results_header').innerHTML = "Resultaten voor "+iq.getFrom();
...
function getSearchForm(iq) {
  if (!iq || iq.getType() != 'result') {
    document.getElementById('searchfields').innerHTML = "Er deed zich een fout voor\nBezig moet afbreken...";
  ...
  html += "<hr noshade size=1><div align=right><button onClick='return searchUser();'>"+"Zoeken"+"</button></div>";
...
onload = init;
  ...
  <body style="margin: 8px;">
		...
		Zoeken op <select name="service" onChange="return serviceSelected(this);"><option value=''>- Map kiezen -</option></select>
		</form>
		...
		<tr><td id="results_header">Bezig met opvragen...</td></tr>
