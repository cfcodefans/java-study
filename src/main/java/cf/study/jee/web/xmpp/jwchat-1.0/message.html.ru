...
function init() {
  ...
  if (user.messages.length > 0) {
    // show messages
    document.title = "Сообщение от "+user.name; // set title
    getNextMSG();
    showMSG();
  }
}
...
onload = init;
  ...
  <body style="margin:8px;">
    ...
    <table width="100%" height="100%" border=0 cellpadding=0 cellspacing=0>
        ...
        <tr id="reply_buttons"><td align="right"><button onClick="return reply(true);">Цитата</button>&nbsp;<button onClick="return reply();">Ответить</button>&nbsp;<button onClick="return openChat();">Начать чат</button></td></tr>

        <tr><td align="right" style="padding-top: 4px;"><button onClick="return prev();" id="prevButton">предыдущий</button>&nbsp;<button onClick="return next();" id="nextButton">следующий</button>&nbsp;<button onClick="window.close();">Закрыть</button></td></tr>
