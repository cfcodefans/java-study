...
function handleIQSetList(iq) {
	...
	if (iq && iq.getType() == 'error') {
		...
		html += "Fout" + " " + error.getAttribute('code');
...
function updateListFrame(modItems) {
	...
	var row = modifylistF.document.createElement("TR");
	var header = new Array("Bijnaam","JID","Boodschap");
	var cell; 
...
function handleIQGetList(iq) {
	if (!iq || iq.getType() == 'error') {
		document.getElementById('hint').innerHTML = "Er deed zich een fout voor";
		if (iq)
			srcW.Debug.log(iq.getDoc().xml,1);
		return;
	}
	...
	if (!iq.getQuery().childNodes.length) {
		document.getElementById('hint').innerHTML = "Er werden geen items gevonden";
	}
...
function init() {
	...
	document.getElementById('title').innerHTML = "Verander "+args[2];
...
onkeydown = keyPressed;
  ...
  <body style="margin:8px;">
		...
		<tr><td align="right">
		<button id="add_button" onClick="return addItem();">Toevoegen</button>&nbsp;<button id="delete_button" onClick="return deleteItem();">Verwijderen</button>&nbsp;<button type="submit" id="save_button" onClick="return doSub();">Opslaan</button>
		</td>
		</tr>
		</table>
  </body>
