...
<html>
  <head>
    <title>JWChat - Chatruimtes zoeken</title>
...
function doSub() {
	...
	if (conference_server.indexOf('@') != -1 || conference_server.indexOf('/') != -1) {
		alert("dit is geen geldige hostnaam");
		return false;
	}
	...
	document.getElementById("search_result_header").innerHTML = "Resultaten voor "+conference_server;
	document.getElementById("search_result_info").innerHTML = "Bezig met opvragen van "+conference_server+". Even geduld alstublieft...";
...
function getDiscoItems(iq) {
	if (!iq || iq.getType() != 'result') {
		document.getElementById("search_result_info").innerHTML = "Er deed zich een fout voor!";
		if (iq)
	...
	var header = new Array("Chatruimte","Beschrijving");
	var cell; 
...
function getVCard(iq) {
	...
	// we're done tell frame about it
		...
		document.getElementById("search_result_info").innerHTML = "Voltooid.";
	}
}
...
onunload = cleanUp;
  ...
  <body style="margin:8px;">
		<div id="search_tab">
			<h2>Chatruimtes zoeken</h2>
			<form name="sub" onsubmit="return doSub();">
				<table width="100%">
						<tr>
							<td nowrap>
								<label for="conference_server">Dienst:
							</td>
							<td width="100%">
		...
		<table id="search_result_tab" width="100%" height="100%">
				...
				<tr>
					<td align=right>
						<button id="addbookmark_button" onClick="return addBookmark();" disabled>Bladwijzer</button>&nbsp;<button id="join_room_button" onClick="return joinRoom();">Betreden</button>&nbsp;<button onClick="return backtosearch();">Terug</button>
					</td>
				</tr>
		</table>
  </body>
