...
<html>
  <head>
    <title>JWChat - Запрос о подписке</title>
...
function init() {
  ...
  document.title = "Запрос на авторизацию от "+jid;

  var from = "<b><span class=\"link\" onClick=\"return srcW.openUserInfo('"+jid+"');\" title=\""+"показать информацию о пользователе "+jid+"\">"+jid.substring(0,jid.indexOf('@'))+"</a></b> ";
...
onload = init;
  ...
  <body style="margin:8px;">
    <table  width="100%" height="100%" border=0 cellpadding=0 cellspacing=0 style="margin-bottom: 8px;">
      <tr><td>
        <span id="requestingUser"></span> желает добавить Вас в свой список:
      </td></tr>
      ...
      <tr><td align="right">
          <button onClick="return sendUnsub();">Запретить</button>&nbsp;<button onClick="return sendSub();" tabindex=1>Разрешить</button>
        </td></tr>
      </form>
    </table>
  </body>
