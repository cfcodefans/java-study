...
function service_selected(el) {
  ...
  pane.innerHTML = "<div class='transport_pane'>"+"Bezig met opvragen van dienst...<br>Even geduld alstublieft!"+"</div>";
...
function getRegisterForm(iq) {
  if (!iq || iq.getType() != 'result' || iq.getQueryXMLNS() != 'jabber:iq:register') {
    pane.innerHTML = "<div class='transport_pane'>"+"Er deed zich een fout voor..."+"</div>";		
    return;
  }
  ...
  if (iq.getNode().getElementsByTagName('x').length && iq.getNode().getElementsByTagName('x').item(0).getAttribute('xmlns') == 'jabber:x:data') {
    ...
    if (query.getElementsByTagName('registered').item(0))
      html += "<div class='pane_el'><b style='color:red;'>"+"U bent al geregistreerd op deze dienst!"+"</b></div>";
  ...
  if (query.getElementsByTagName('registered').item(0)) {
    html += "<button onClick=\"unregisterGateway('"+iq.getFrom()+"');\">"+"Registratie ongedaan maken"+"</button>";
    html += "&nbsp;";
    html += "<button onClick=\"registerService('"+iq.getFrom()+"','"+pane.id+"');\">"+"Bijwerken"+"</button>";
  } else
    html += "<button onClick=\"registerService('"+iq.getFrom()+"','"+pane.id+"');\">"+"Registreren"+"</button>";
  html += "</div>";
  pane.innerHTML = html;
...
function registerService(sJid,paneid) {
  ...
  pane.innerHTML = "<strong>"+"Aanvraag verzonden naar "+sJid+"..."+"</strong><br>";
}
...
function handleRegisterService(iq,sJid) {
  if (iq.getType() == 'error') {
    ...
    var html = "<strong style='color:red;'>"+"Fout";
	...
	case 'bad-request': html += "Foutieve aanvraag";
	  break;
  ...
  pane.innerHTML += "<strong>"+"Registratie voltooid."+"</strong><br/>";
	
  // send presence
  pane.innerHTML += "<strong>"+"Bezig met verzenden van aanwezigheid..."+"</strong><br />";
  var aPresence = new JSJaCPresence();
  aPresence.setTo(sJid);
  aPresence.setShow(srcW.onlstat);
  aPresence.setStatus(srcW.onlmsg);
  srcW.con.send(aPresence);
  pane.innerHTML += "<strong>"+"Klaar."+"</strong><br />";
}
...
function unregisterGateway(sJid) {
  ...
  pane.innerHTML = "<strong>"+"Bezig met ongedaan maken van registratie op de dienst "+sJid+" ..."+"</strong><br />";
  var iq = new JSJaCIQ();
  iq.setType('set');
  iq.setTo(sJid);
  var query = iq.setQuery('jabber:iq:register');
...
function handleUnregisterService(iq,sJid) {
  ...
  pane.innerHTML += "<strong>"+"Klaar."+"</strong><br />";
  pane.innerHTML += "<strong>"+"Bezig met verwijderen van "+sJid+" van roster..."+"</strong><br />";
  ...
  if (confirm("Wilt u werkelijk alle betrokken contactpersonen verwijderen?")) {
    pane.innerHTML += "<strong>"+"Bezig met verwijderen van contactpersonen..."+"</strong><br />";
    var iq = new JSJaCIQ();
    iq.setType('set');
    var query = iq.setQuery('jabber:iq:roster');
    for (var i=0; i<srcW.roster.users.length; i++) {
...
onunload = savePrefs;
  ...
  <body style="margin: 8px;">
	...
	<table border=0 cellpadding=0 cellspacing=0 height="100%" width="100%">
		<tr>
			<td>
			<div id="tablabels" class="tablabels"><span class="tablabelactive">Algemeen</span><span class='tablabel' id='pw_label'>Change Password</span><span class="tablabel" id='gw_label'>Transporten</span><span class="tablabel" id='dir_label'>Gebruikersdatabase</span><span class="tablabel">Over</span></div>
			</td></tr>
					...
					<fieldset>
						...
						<table border="0">
								<tr>
									...
									<td><label for="usersHidden">Offline contactpersonen verbergen</label></td>
								</tr>
								<tr>
									<td><input id="autoPopup" type="checkbox" onChange="srcW.autoPopup=this.checked;toggleAutoPopupAway(this);"></td>
									<td><label for="autoPopup">Normale berichten en chatberichten automatisch openen</label></td>
								</tr>
								<tr>
									<td><input id="autoPopupAway" type="checkbox" onChange="srcW.autoPopupAway=this.checked"></td>
									<td><label for="autoPopupAway">Inkomende berichten in een nieuw venster tonen bij de aanwezigheid "afwezig"</label></td>
								</tr>
								<tr>
									<td><input id="playSounds" type="checkbox" onChange="srcW.playSounds=this.checked;"></td>
									<td><label for="playSounds">Geluiden afspelen</label></td>
								</tr>
								<tr>
									<td><input id="focusWindows" type="checkbox" onChange="srcW.focusWindows=this.checked;"></td>
									<td><label for="focusWindows">Venster naar de voorgrond brengen bij ontvangst van een nieuw bericht</label></td>
								</tr>
								<tr>
									<td><input id="timestamps" type="checkbox" onClick="displayTimestamps();"></td>
									<td><label for="timestamps">Tijdsaanduidingen in chatvensters tonen</label></td>
								</tr>
								<tr id='tr_enableLog'>
									<td><input id="enableLog" type="checkbox" onChange="srcW.enableLog=this.checked;"></td>
									<td><label for="enableLog">Berichten bewaren</label></td>
								</tr>
<script language="JavaScript">
			...
			</div><div class="tabinactive" id="transports_tab">Registreren op <select id="transports_selector" onchange="service_selected(this);"><option value=""> - Transport kiezen - </option></select><div id="transport_pane"></div></div><div class="tabinactive" id="directory_tab">Registreren op  <select id="directory_selector" onchange="service_selected(this);"><option value=""> - Map kiezen - </option></select><div id="directory_pane"></div></div><div class="tabinactive" id="about_tab" align="center">
								<p>
