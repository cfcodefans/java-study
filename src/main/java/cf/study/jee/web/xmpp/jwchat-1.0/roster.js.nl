...
function RosterUpdateGroupForUser(user) {
  for (var j=0; j<user.groups.length; j++) {
    if (user.groups.length > 1 && user.groups[j] == '')
      continue;
    var groupName = (user.groups[j] == '') ? "Leeg" : user.groups[j];
...
function printRoster() {
  ...
  for (var i=0; i<this.groups.length; i++) {
    ...
    var rosterGroupHeadClass = (this.usersHidden && 
				this.groups[i].onlUserCount == 0 && 
				this.groups[i].messagesPending == 0 && 
				this.groups[i].name != "Transporten") 
      ...
      if (user.statusMsg) {
        A[A.length] = "&#10;";
			...
			if ((user.messages.length + user.chatmsgs.length) > 0) {
				A[A.length] = "&#10;";
				A[A.length] = ""+(user.messages.length + user.chatmsgs.length)+" bericht(en) in behandeling";
			}
      A[A.length] = "\">";
...
function getRosterUserClass(usersHidden, user, group) {
  return (usersHidden && (user.status == 'unavailable' || user.status == 'stalker') && !user.lastsrc && group.name != "Transporten" ?
          "hidden" : "rosterUser");
}

function getRosterGroupHeaderClass(usersHidden, group) {
  return (usersHidden && group.onlUserCount == 0 && group.messagesPending == 0 && group.name != "Transporten" ?
         'rosterGroupHeaderHidden':'rosterGroupHeader');
}
...
function getUserElementTitle(user) {
  ...
  if ((user.messages.length + user.chatmsgs.length) > 0) {
    elTitle += "&#10;";
    elTitle += ""+(user.messages.length + user.chatmsgs.length)+" bericht(en) in behandeling";
  }
  return elTitle;
}
