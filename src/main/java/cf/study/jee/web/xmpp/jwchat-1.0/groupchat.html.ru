...
function updateMe() {
  ...
  if (roster.me.role == 'none') {// seems we left
    cFrame.body.innerHTML += "<span style='color:red';>"+"Отключено."+"</span><br>";
    frames.groupchatChat.groupchatIChat.scrollTo(0,cFrame.body.scrollHeight);
  }
  ...
  if (iq && iq.getType() == 'error') {
    ...
    if (error) {
      ...
      for (var i=0; i<error.childNodes.length; i++) {
        ...
        case 'not-allowed':
          putMsgHTML("Не разрешено",new Date(),group,null,true);
          break;
        case 'forbidden':
          putMsgHTML("Запрещено",new Date(),group,null,true);
          break;
        case 'item-not-found':
          putMsgHTML("Не найдено",new Date(),group,null,true);
          break;
        default:
...
function init() {
  ...
  user = srcW.roster.getUserByJID(group);
  if(!user) {
    user = srcW.roster.addUser(new RosterUser(group,'',["Комнаты чата"],group.substring(0,group.indexOf('@'))));
