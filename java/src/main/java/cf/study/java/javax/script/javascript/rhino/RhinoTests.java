package cf.study.java.javax.script.javascript.rhino;
...
public class RhinoTests {
	...
	public static Logger log = Logger.getLogger(RhinoTests.class.getSimpleName());
	
	public static String getScript(final String fileName) {
		try {
			return IOUtils.toString(RhinoTests.class.getResourceAsStream(fileName));
		} catch (IOException e) {
	...
	public static Object runScript(final String scriptStr) throws Exception {
		...
		try {
			...
			final Script script = cx.compileString(scriptStr, "", 1, null);
			return script.exec(cx, sharedScope);
		} finally {
			Context.exit();
		}
	}
    ...
    */
	...
	public void fooTest() throws Exception {
		RhinoTests.runScript(getScript("foo.js"));
	}
...
//		RhinoTests.runScript(getScript("wrap_factory.js"));
		...
		final WrapFactory wf = Context.getCurrentContext().getWrapFactory();
		System.out.println(wf.isJavaPrimitiveWrap());
